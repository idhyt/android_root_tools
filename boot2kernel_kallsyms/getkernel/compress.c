...
size_t lzma(int mode, int fd, const void *buf, size_t size) {
	...
	if (ret != LZMA_OK)
		fprintf(stderr, "Unable to init lzma stream\n");
...
size_t lz4(int mode, int fd, const void *buf, size_t size) {
	...
	if (LZ4F_isError(ret))
		fprintf(stderr, "Context creation error: %s\n", LZ4F_getErrorName(ret));
	...
	switch(mode) {
		case 0:
			...
			if (LZ4F_isError(ret))
				fprintf(stderr, "LZ4F_getFrameInfo error: %s\n", LZ4F_getErrorName(ret));
			switch (info.blockSizeID) {
				...
				default:
					fprintf(stderr, "Impossible unless more block sizes are allowed\n");
			}
			pos += read;
			break;
		case 1:
	...
	if (mode == 1) {
		...
		if (LZ4F_isError(ret))
			fprintf(stderr, "Failed to start compression: error %s\n", LZ4F_getErrorName(ret));
		total += xwrite(fd, out, have);
	}
	...
	do {
		...
		do {
			...
			if (LZ4F_isError(ret))
				fprintf(stderr, "LZ4 coding error: %s\n", LZ4F_getErrorName(ret));
	...
	switch(mode) {
		...
		case 1:
			...
			if (LZ4F_isError(ret))
				fprintf(stderr, "Failed to end compression: error %s\n", LZ4F_getErrorName(ret));
...
size_t bzip2(int mode, int fd, const void* buf, size_t size) {
	...
	if (ret != BZ_OK)
		fprintf(stderr, "Unable to init bzlib stream\n");
	...
	do {
		switch(mode) {
			case 0:
				...
				if (have < 0)
					fprintf(stderr, "Cannot decode lz4_legacy block\n");
				pos += block_size;
				break;
			case 1:
				...
				if (have == 0)
					fprintf(stderr, "lz4_legacy compression error\n");
				pos += insize;
				// Write block size
...
void decomp_file(char *from, const char *to) {
	...
	if (ext != NULL) {
		...
		case LZ4:
			...
			break;
		default:
			fprintf(stderr, "Provided file \'%s\' is not a supported archive format\n", from);
		}
		if (strip)
			*ext = '\0';
	}
