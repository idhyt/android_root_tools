...
def write_bootimg(output, kernel, ramdisk, second, dtimg,
    ...
    def get_size(fd):
        ...
        assert hasattr(fd, 'tell')
        fd.seek(0, 2)
        return fd.tell()

    def write_content(out_put, fd):
        if fd is None:
            # sha.update('')
        ...
        out_put.write(content)
        out_put.write(padding(fd.tell()))

        if hasattr(fd, 'close'):
            fd.close()

    output.write(struct.pack('<8s10I16s512s', 'ANDROID!',
                             get_size(kernel), kernel_addr,
                             get_size(ramdisk), ramdisk_addr,
                             get_size(second), second_addr,
                             tags_addr, page_size, get_size(dtimg), 0,
                             name, cmdline))
    ...
    sha = sha1()
    write_content(output, kernel)
    write_content(output, ramdisk)
    write_content(output, second)
    write_content(output, dtimg)
    # fill back sha1
    output.seek(id_pos, 0)
        ...
        if bytecount == 0xffff:
            ...
            if filetype == 0:  # unknown
                ...
                try:
                    ...
                    sys.stderr.write('directory mode is not supported')
            elif filetype == 4:  # hlink
                link = filelist.get(equivalent)
                try:
                    os.link(filelist.get(equivalent), fullname)
                except:
                    sys.stderr.write('hard %s -> %s\n' % (fullname, link))
...
def repack_bootimg(kernel_addr=None, ramdisk_addr=None, second_addr=None, tags_addr=None, cmdline=None, page_size=None,
                   padding_size=None):
    ...
    sys.stderr.write(
        'arguments: [kernel_addr] [ramdisk_addr] [second_addr] [tags_addr] [cmdline] [page_size] [padding_size]\n')
    metadata = {}
